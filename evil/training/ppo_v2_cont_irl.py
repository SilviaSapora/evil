...
import distrax
from evil.training.wrappers import (
    ClipActionRewardIRL,
    LogWrapperRewardIRL,
...
def eval(
    ...
    if normalize_obs:
        # if "mean" in env_state_norm:
        env_state_mean = env_state_norm.mean
        env_state_var = env_state_norm.var
        # else:
        #     env_state_mean = env_state_norm[0]
        #     env_state_var = env_state_norm[1]
    else:
        env_state_mean = jnp.zeros_like(obsv)
        env_state_var = jnp.ones_like(obsv)
    if normalize_obs:
        norm_obsv_init = (obsv - env_state_mean) / jnp.sqrt(env_state_var + 1e-8)
    else:
        norm_obsv_init = obsv
    ...
    def _env_step(runner_state, unused):
        env_state, last_obs, rng, prev_done = runner_state
        ...
        prev_done = done
        if normalize_obs:
            norm_obsv = (obsv - env_state_mean) / jnp.sqrt(env_state_var + 1e-8)
        else:
            norm_obsv = obsv
        runner_state = (env_state, norm_obsv, rng, prev_done)
        return runner_state, (done, last_obs, action, reward)

    rng, _rng = jax.random.split(rng)
    runner_state = (env_state, norm_obsv_init, _rng, prev_done)
...
def make_train(
    ...
    env = ClipActionRewardIRL(env)
    env = VecEnvRewardIRL(env)
    if "NORMALIZE_ALL_REWARD" not in config:
        config["NORMALIZE_ALL_REWARD"] = False
    env = NormalizeVecRewardIRL(
        env, config["GAMMA"], config["NORMALIZE_REWARD"], config["NORMALIZE_ALL_REWARD"]
    )
    ...
    def train(rng):
        ...
        if runner_state_start is None:
            ...
            norm_obsv, obsv, env_state = env.reset(reset_rng, env_params)
            prev_done = jnp.ones(shape=(config["NUM_ENVS"],), dtype=jnp.bool_)
            rng, _rng = jax.random.split(rng)
            runner_state = (train_state, env_state, norm_obsv, _rng, prev_done)
        else:
        ...
        def _update_step(runner_state, unused):
            ...
            traj_batch.info["timestep_returned_episode_returns"] = traj_batch.info[
                "timestep_returned_episode_returns"
            ]
            traj_batch.info["timestep_returned_episode_irl_returns"] = traj_batch.info[
                "timestep_returned_episode_irl_returns"
            ]
        ...
        if log_timestep_returns:
            if return_obs_and_actions > 0:
                runner_state, (metric, actor_obs, actor_actions) = jax.lax.scan(
                    _update_step, runner_state, None, config["NUM_UPDATES"]
                )
                actor_obs = actor_obs[:, :, :return_obs_and_actions, :]
                actor_actions = actor_actions[:, :, :return_obs_and_actions, :]
            else:
                runner_state, (metric, _, _) = jax.lax.scan(
                    _update_step, runner_state, None, config["NUM_UPDATES"]
                )
                actor_obs = None
                actor_actions = None
            ...
            if return_obs_and_actions > 0:
                ...
                runner_state, (_, _, _) = jax.lax.scan(
                    _update_step, runner_state, None, config["NUM_UPDATES"]
                )
                metric = {}
                actor_obs = None
                actor_actions = None
        metric["returned_episode_real_returns"] = runner_state[
            1
        ].env_state.env_state.returned_episode_real_returns
        metric["real_auc"] = (
            runner_state[1].env_state.env_state.sum_of_real_returns
            / runner_state[1].env_state.env_state.timestep
        )
        metric["irl_auc"] = (
            runner_state[1].env_state.env_state.sum_of_irl_returns
            / runner_state[1].env_state.env_state.timestep
        )
        metric["avg_episode_irl_return"] = runner_state[
            1
        ].env_state.env_state.sum_of_irl_returns / (
            runner_state[1].env_state.env_state.dones + 1
        )
        metric["avg_episode_real_return"] = runner_state[
            1
        ].env_state.env_state.sum_of_real_returns / (
            runner_state[1].env_state.env_state.dones + 1
        )
        metric["returned_episode_irl_returns"] = runner_state[
            1
        ].env_state.env_state.returned_episode_irl_returns
        if runner_state_start is not None:
            metric["diff_returned_episode_real_returns"] = (
                runner_state[1].env_state.env_state.sum_of_real_returns
                - start_sum_of_real_returns
            ) / start_timestep
        else:
            metric["diff_returned_episode_real_returns"] = metric[
                "returned_episode_real_returns"
            ]
        metric["last_real_return"] = runner_state[
            1
        ].env_state.env_state.returned_episode_real_returns
        metric["last_irl_return"] = runner_state[
            1
        ].env_state.env_state.returned_episode_irl_returns

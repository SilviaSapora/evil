...
def eval(
    ...
    def _env_step(runner_state, unused):
        ...
        prev_done = done
        runner_state = (env_state, obsv, rng, prev_done)
        return runner_state, (done, last_obs, action, reward)

    rng, _rng = jax.random.split(rng)
    runner_state = (env_state, obsv, _rng, prev_done)
    ...
    def train(rng):
        ...
        if runner_state_start is None:
            ...
            start_sum_of_real_returns = runner_state_start[
                1
            ].env_state.sum_of_real_returns
            start_sum_of_irl_retuns = runner_state_start[1].env_state.sum_of_irl_returns
            ...
            logging_env_state = runner_state_start[1].env_state.replace(
                ...
                sum_of_irl_returns=jnp.zeros_like(start_sum_of_irl_retuns),
        ...
        metric["returned_episode_real_returns"] = runner_state[
            1
        ].env_state.returned_episode_real_returns
        metric["returned_episode_irl_returns"] = runner_state[
            1
        ].env_state.returned_episode_irl_returns
        metric["real_auc"] = (
            runner_state[1].env_state.sum_of_real_returns.sum()
            / runner_state[1].env_state.dones.sum()
        )
        if runner_state_start is not None:
            metric["diff_returned_episode_real_returns"] = (
                runner_state[1].env_state.sum_of_real_returns

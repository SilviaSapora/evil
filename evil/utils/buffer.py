...
import jax
from evil.utils.utils import maybe_concat_action
...
class ObsvActionBuffer:
    ...
    def add(self, obsv, actions, key, state):
        ...
        new_obsv_buffer = jnp.concatenate((state.buffer_obsv, chosen_obsv), axis=0)[
            -self.max_size :
        ]
        new_actions_buffer = jnp.concatenate(
            (state.buffer_actions, chosen_actions), axis=0
        )[-self.max_size :]
    ...
    def sample(self, num_samples, key, state):
        # dist = distrax.Beta(1, 2)
        # vals = dist.sample(seed=key, sample_shape=(num_samples,))
        # chosen_idx = (vals * (state.buffer_size)).astype(jnp.int32)
        chosen_idx = jax.random.randint(
            key, shape=(num_samples,), minval=0, maxval=state.buffer_size
        )
        obsv, actions = (
            state.buffer_obsv[-chosen_idx],
            state.buffer_actions[-chosen_idx],
        )
    ...
    def sample_recent(self, num_samples, key, state):
        chosen_idx = jax.random.randint(
            key,
            shape=(num_samples,),
            minval=0,
            maxval=(self.num_envs * 10),
        )
        obsv, actions = (
            state.buffer_obsv[-chosen_idx],
            state.buffer_actions[-chosen_idx],

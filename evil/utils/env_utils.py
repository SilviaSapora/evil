...
from evil.envs.gridworld import GridWorldNew
...
from evil.envs.gridworld import EnvParams as GridWorldEnvParams
import jax.numpy as jnp
from evil.configs.inner_training_configs import (
    CARTPOLE_CONFIG,
    GRIDWORLD_CONFIG,
    BRAX_CONFIG,
    PENDULUM_CONFIG,
...
def get_env(env_name, backend="positional"):
    if is_brax_env(env_name):
        env, env_params = BraxGymnaxWrapper(env_name, backend=backend), None
...
def get_gridworld_env():
    ...
    envs_params = GridWorldEnvParams(mazes)
    return env, envs_params


def get_test_params(es_config):
    if es_config["env"] == "Reacher-misc":

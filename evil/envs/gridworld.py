...
def get_state_dict_from_obs(num_columns, num_rows, num_rewards, obs):
    obs = obs.reshape(num_columns, num_rows, -1)
    agent_pos = jnp.asarray(jnp.nonzero(obs[:, :, 1], size=(1), fill_value=num_columns))
    found = jnp.array([False] * num_rewards)
    distance = jnp.array([num_columns + num_rows] * num_rewards)
    goals = None
    for num_reward in range(2, num_rewards + 2):
        reward = jnp.asarray(
            jnp.nonzero(obs[:, :, num_reward], size=(1), fill_value=num_columns)
        )
        if reward[0] >= num_rows or reward[1] >= num_columns:
            found = found.at[num_reward - 2].set(True)
            distance = distance.at[num_reward - 2].set(0)
        else:
            found = found.at[num_reward - 2].set(False)
            cur_distance = abs(agent_pos[0][0] - reward[0][0]) + abs(
                agent_pos[1][0] - reward[1][0]
            )
            distance = distance.at[num_reward - 2].set(cur_distance)
        if goals is None:
            goals = reward
        else:
            goals = jnp.concatenate([goals, reward], axis=0)
    return {"pos": agent_pos, "goals": goals, "found": found, "distance": distance}
...
#         rng1, shape=(), minval=0, maxval=horizontal_wall_x
#     )
#     vertical_wall_door_down = jax.random.randint(
#         rng2, shape=(), minval=horizontal_wall_x + 1, maxval=rows
#     )
#     horizontal_wall_door_left = jax.random.randint(
#         rng3, shape=(), minval=0, maxval=vertical_wall_y
#     )
#     horizontal_wall_door_right = jax.random.randint(
#         rng4, shape=(), minval=vertical_wall_y + 1, maxval=columns
#     )
...
class GridWorldNew(environment.Environment):
    ...
    def get_obs(self, state: EnvState) -> chex.Array:
        return jnp.concatenate(
            (
                state.pos / self.columns,
                state.goals.flatten() / self.columns,
                state.found.astype(int),
            ),
            axis=-1,
        )
...
def sample_init_state(
    ...
    pos_indexes = get_random_position(grid_env, grid_indexes, num_rewards + 1, rng)
    # pos_index = jnp.array([2, 0])
    # goal_indexes = get_random_position(grid_env, grid_indexes, num_rewards, next(rng))
    goal_indexes = jnp.array([[0, 4]])
    # goal_indexes = pos_indexes[1:, :]
    # return goal_indexes, pos_indexes
